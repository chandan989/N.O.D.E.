...
const NewsCard = ({ article }) => (
    <div className="flex items-center space-x-4 p-2 rounded-lg hover:bg-gray-200 cursor-pointer">
        ...
        <div>
            <p className="font-semibold text-black text-sm">{article.headline}</p>
            <p className="text-xs text-gray-500">{article.source} - <span className="text-black font-mono">{article.tickers.join(', ')}</span></p>
        </div>
    </div>
);
...
const LearningItem = ({ resource }) => (
    <div className="p-2 rounded-lg hover:bg-gray-200 cursor-pointer">
        <p className="font-semibold text-sm text-black">{resource.title}</p>
...
export const NewsAndLearning = ({ isExpandable, isExpanded, onToggleExpand }) => {
  ...
  return (
    <div className="pixel-card p-6 space-y-6">
      <div>
        <h4 className="text-lg font-bold mb-4 text-black">LATEST NEWS</h4>
      ...
      <div className="pt-4 border-t-2 border-dashed border-black/20">
        <h4 className="text-lg font-bold mb-4 text-black">LEARNING HUB</h4>
